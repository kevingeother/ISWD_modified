...
def line_to_words(line, min_len, max_len=None, stop_words=None, lemmatize=True):
    """
    Reads a line of text (sentence) and returns a list of tokenized EDUs
    return:
        segs: word list processed by clean_str, stopwords, lemmatize
        original: text split by 'EDU_BREAK'
    """
    if lemmatize:
        ...
        if not first_line:
            if len(line.strip()) != 0:
                ...
                for i, seg in enumerate(segs):
                    ...
                    scode = '{0}-{1:04d}'.format(rcode, start_idx + ids[i])     # unique index for each segment
...
def clean_str(string):
    ...
    string = re.sub(r"(?<= )[0-9]+(?= )", "<NUM>", string)  # use <NUM> to replace number
    string = re.sub(r"\s{2,}", " ", string) # replace multiple space with " "
    return string.strip()
...
def main():
    ...
    # args.name = "./data/preprocessed/BOOTS_MATE"
    # args.data = "./data/train/boots.trn"
    # args.lemmatize = True
    ...
    print("start loading w2v")  # w2v files are already given
    # vocab_size = len(word2id) + 1
    # w2v = load_bin_vec(args.w2v, word2id)     # ISSUE
    # embed = np.random.uniform(-0.25, 0.25, (vocab_size, len(w2v.values()[0])))
    # embed[0] = 0      # embedding for <PAD>
    ...
    data, products, scodes, original = zip(*sorted(list(zip(data, products, scodes, original)), key=lambda x:len(x[0])))

    print("start writing h5py")
    ...
    with h5py.File(filename, 'w') as f:
        ...
        for i, (segments, prods, codes, segs_o), in enumerate(parallel_chunks(data, products,
            ...
            # f.create_dataset('scodes/' + batch_id, (len(codes),), data=codes) # dtype="S{0}".format(len(codes[0])), data=codes)
            f.create_dataset('scodes/' + batch_id, (len(codes),), dtype="S{0}".format(len(codes[0])), data=codes)
            dt = h5py.special_dtype(vlen=bytes)
            # f.create_dataset('original/' + batch_id, (len(segs_o),), data-segs_o)  # dtype=dt, data=segs_o)
