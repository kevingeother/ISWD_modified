...
def smooth_cross_entropy(input, target, size_average=True):
    ...
    """
    '''
    input is not yet through softmax, so use logsoftmax here
    '''
...
class EmbeddingEncoder(nn.Module):
    def __init__(self, vocab_size, emb_size, fix_w_emb=False, pretrained_emb=None):
        ...
        self.encoder = nn.EmbeddingBag(vocab_size, emb_size)    # mode='mean'

        if pretrained_emb is None:
            xavier_uniform_(self.encoder.weight.data)
        else:
...
class AttentionEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, emb_size, bias=True, M=None, b=None, pretrained_emb=None, fix_w_emb=False):
        ...
        if pretrained_emb is None:
            ...
            assert pretrained_emb.size() == (vocab_size, emb_size), \
                "Word embedding matrix has incorrect size: {} instead of {}".format(pretrained_emb.size(), (vocab_size, emb_size))
...
class SeedCLF:
    def __init__(self, id2word, aspects_ids, seed_weights=None, verbose=0, general_ind=4, general_thres=0,
        ...
        self.seed_dict = self.create_seed_dict()    # {seedword_id: (aspect_id, seed_weight)}
    ...
    def create_seed_dict(self):
        '''
        Note: if there are duplicated seed words, in this implementation the latter one would cover the former one
        '''
    ...
    def predict(self, seg):
        '''
        return:
            aspect_probs: not probability, just seed_weight counter
        '''
        seg = list(seg)
...
class StudentBoWCLF(nn.Module):
    ...
    '''
    use bag-of-word model as presentation vector of a segment 
    '''
...
class AspectCLF(nn.Module):
    '''
    basic components: seg_encoder (EmbeddingEncoder/AttentionEncoder), dropout_layer, batch_norm, seed_encodings, aspect_clf

    deep_clf == MLP: ids -> pad_sequences -> seg_encoder -> linear -> bn -> relu -> linear -> bn -> relu -> enc
    deep_clf == CNN: ids -> deep_encoder (CNN_encoder) -> enc
    use_bert: bert_embeddings [-> [linear, bn, relu] * 2] -> enc

    enc -> dropout -> aspect_clf -> a_logits

    default value from Trainer and cmd arguments:
        fix_w_emb: False
        fix_a_emb: False
        pretrained_emb: from datahandler
        seed_encodings: None
        seed_weights: None
        num_seeds: 30
        attention: False
    ...
    default pipeline for w2v
        seg_ids -> pad_sequences -> seg_encoder=EmbeddingEncoder -> dropout_layer=nn.Dropout() -> aspect_clf=nn.Linear()
    default pipeline for bert
        inputs['bert_embeddings'] -> dropout_layer -> aspect_clf
    '''
    def __init__(self, vocab_size=3000, num_aspects=9, emb_size=200, enable_gpu=True, cuda_device=0,
        ...
        if seed_encodings is None:
            ...
            if self.seed_weights is None:
                ...
                seed_encodings = seed_encodings.mean(dim=1)     # [n_asp, n_seedword, emb_size] -> [n_asp, emb_size]
            else:
        ...
        if seed_encodings is not None:
            ...
            xavier_uniform_(self.aspect_clf.weight.data)    # initialize the classifier
    ...
    def forward(self, inputs):
        ...
        if self.deep_clf == 'MLP' and not self.use_bert:
            ...
            enc = inputs['bert_embeddings']     # already embedding for segments
