...
class Trainer:
    '''
    w2v:
        pairs, lr=0.0005, no_seed_weights, emb_dropout=0.5, weight_decay=0.1, loss=SmoothCrossEntropy
    bert:
        pairs, lr=0.00005, no_seed_weights, emb_dropout=0.5, weight_decay=0.0001, use_bert

    pipeline: [start_epoch() -> train() -> validate() -> validate_test() -> end_epoch()] * epochs -> test() -> close()
    
    variable:
        epoch_results = {'train': {'acc', 'prec', 'rec', 'f1', 'macro_average_f1', 'micro_average_f1', 'conf_mat', 'loss'}, 
                         'valid': , 
                         'test': }
        results = [epoch_results_{epoch}]
    '''
    def __init__(self, args):
        ...
        pretrained_emb = self.datahandler.w_emb     # load from hdf5 file
    ...
    def train(self):
        ...
        metrics are compared with teacher predictions
        train()/validate()/validate_test() share the similar structure: 
            compute loss, calculate metrics results, add_histogram
        '''
        ...
        for batch in self.datahandler.get_train_batches():
            ...
            if self.args.loss not in ["SmoothCrossEntropy", "KL"]:
                ...
                label = F.normalize(label, p=1, dim=1)      # not softmax, but normalize!
            loss = self.loss_fn(pred, label)
            all_losses.append(loss.data.cpu().numpy())
            loss.backward() 
            self.optimizer.step()

            all_preds = torch.cat((all_preds, pred.data), dim=0)    # [N, n_asp]
        ...
        all_proba = all_preds.cpu().numpy()     # [B, n_asp]
        max_prob, all_preds = all_preds.max(dim=1)
        all_preds = all_preds.cpu().numpy() # [B]
    ...
    def validate(self):
        '''
        loss is already computed w.r.t. true labels
        '''
    ...
    def test(self, savename='results.pkl'):
        ...
        all_preds = all_preds.cpu().numpy()
        res = self.evaluator.evaluate(all_preds, all_labels, all_proba, gt_classes=range(self.args.num_aspects), verbose=False)
        self.epoch_results['test'] = res    # FIXME: not the correct place for storing results? cover the last epoch valid_test result
    ...
    def end_epoch(self):
        '''
        add metrics result from each epoch to SummaryWriter
        metrics from training data are not used, except training loss
        plotting metrics:
            loss_{domain}/train_loss
            loss_{domain}/valid_loss
            micro_average_f1/{domain}
            test_micro_average_f1/{domain}
            average_test_results/{acc, avg_prec, avg_rec, macro_average_f1, micro_average_f1}
            test_results/{} -> provide separate view into different aspects
            test_conf_matrix

        my opinion: quiet messy
        '''
        # Do necessary stuff at the end of each epoch
        self.writer.add_scalars('loss{}'.format(self.comment), {
            'train_loss': self.epoch_results['train']['loss'],  # average loss
        ...
        self.writer.add_scalars('test_' + self.metric, {self.args.domain: score}, self.epoch)   # FIXME: should be test_score?

        res_flattened = self.flattened_test_result_dict     # validate_test
        res_flattened['avg_prec'] = np.average(self.epoch_results['valid']['prec'])     # average over different aspects
        res_flattened['avg_rec'] = np.average(self.epoch_results['valid']['rec'])   # FIXME: should be self.epoch_results['test']?
    ...
    def close(self):
        '''
        after all epochs
        example logdir: $logdir/$(date +'%h%d_%H-%M-%S-%N')_W2V/v_{}/{domain}
        `epoch_results.pkl` and `results.pkl` are the same content
        '''
        self.writer.close()
...
def report_results(logdir, domains, metric='micro_average_f1'):
    ...
    with open(logdir + '/results.txt', 'w') as f:
        ...
        for domain in domains:
            ...
            res_file = logdir + '/' + domain + '/' + 'epoch_results.pkl'    # results from all epochs
            if not os.path.exists(res_file):
                print("[ERROR] Could not find {}".format(res_file))
                continue
            results[domain] = joblib.load(res_file)
            all_scores = [x['valid'][metric] for x in results[domain]]  # results from all epochs, valid set micro_f1
...
'''
main() -> run_trainer() (parallel)      -> report_results()
            |-> initialize Trainer for different domain
            |-> trainer.process()
'''