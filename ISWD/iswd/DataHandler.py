...
def load_data_oposum(domain='bags_and_cases', num_seeds = 30):
    ...
    return:
        word2id: vocab
        id2word: 
        aspects_ids: [n_asp, n_seed], vocab index of each seedword
        seed_weights: [n_asp, n_seed]
        aspect_names: list of aspect names
        aspect2id: {aspect: id} (from 0 to n_asp-1)
    '''
...
def load_wordemb_oposum(fpath):
    # word2id already done in `load_data_oposum`
    id2word = {}
    word2id = {}
...
def get_aspect_seeds_oposum(aspect_seeds, word2id):
    ...
    if seed_weights is not None:
        ...
        seed_w /= seed_w.norm(p=1, dim=1, keepdim=True) # norm -> sum of absolute values
    else:
        seed_w = None
...
def get_aspect_embeddings_oposum(aspect_seeds, w_emb_array, word2id):
    ...
    for seeds in aspects_ids:
        clouds.append(w_emb_array[seeds])   # a list of arrays
    a_emb = torch.from_numpy(np.array(clouds))  # [n_asp, n_seed, dim]
    return a_emb, seed_w
...
def get_weak_labels(domain, num_seeds, use_seed_weights=True, pretrained_teacher_folder=''):
    '''
    teacher prediction, seems that `pretrained_teacher_folder` default to ''
    return:
        train_labels: pesudo label
    '''
    ...
    if pretrained_teacher_folder == '':
        ...
        train_labels = [seedCLF.predict(x) for x in train_segs]     # [B, n_asp]
    else:
        '''
        in this code, there is no way to enter this branch, 
            because `pretrained_teacher_folder` is fixed to '' and not updated during training 
        '''
...
def get_labeled_train_data(domain, num_seeds, one_hot=False, use_seed_weights=True, pretrained_teacher_folder=''):
    '''
    data are sorted by index length
    '''
...
def get_pretrained_bert_embeddings(domain, scodes, method='train', bert_model='base'):
    '''
    pre-computed BERT embedding for each segment in train/dev/test dataset
    '''
...
class DataHandler:
    def __init__(self, args):
        '''
        Variables:
            load_data -> word2id, id2word, aspects_ids, seed_weights, aspect_names, aspect2id
            general_ind
            load_dev_segments -> dev_segs, dev_labels, dev_scodes
            load_test_segments -> test_segs, test_labels, test_scodes
            get_labeled_train_data -> (**length-sorted**) train_segs, train_labels, train_original, train_scodes 
            load_wordemb -> word2id, w_emb
            get_aspect_embeddings -> a_emb, seed_w
            get_pretrained_bert_embeddings -> {train, dev, test}_bert_embeddings 
        Note:
            1. seed_weight for each aspect are supposed to sum up to 1 
            2. training data are already sorted by length, dev and test data use batch_size 1
        **IMPORTANT**
            1. there is no seedword weight update in the code, `pretrained_teacher_folder` is fixed in this code
        '''
        ...
        self.flat_aspect_ids = set([i for a in self.aspects_ids for i in a])    # also consider overlapping seedwords
        ...
        if self.domain in oposum_domains:
            ...
            raise (BaseException('domain not available: {}'.format(self.domain)))
    ...
    def get_train_batches(self):
        ...
        for i, batch_st in enumerate(range(0, total, batch_size)):
            ...
            yield {
                'ind': i,
                'ids': np.array(batch_segs, dtype=object),  # without `dtype=` there is VisibleDeprecationWarning
                'teacher_ids': np.array(batch_teacher_segs, dtype=object),
